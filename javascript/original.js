...
let sap_ratio_global_original = 0;
let alkali_ratio_global_original = 0;
let alcohol_ratio_global_original = 0;
...
const calc_alkali_original = (oils, total, ratios) => {
    ...
    // 固形せっけん固定
    const alkali = calc_soda_original(oils, total, ratios, discount, alkali_rate);
    sap_ratio_global_original = discount;
    alkali_ratio_global_original = alkali_rate;
    alcohol_ratio_global_original = alkali_rate;
...
const get_oil_names_original = (oils, total, ratios) => {
    let oil_names = [];
...
const get_option_names_original = (options) => {
    let option_names = [];
    if (options.length > 0) {
...
const get_features_original = (oils, total, ratios) => {
    let skin = 0;
    let clean = 0;
    let foam = 0;
    let hard = 0;
...
const get_option_adjustments_original = (options) => {
...
const get_final_features_original = (oils, total, ratios, options) => {
    let features = get_features_original(oils, total, ratios);
    let option_adjustments = get_option_adjustments_original(options);
...
const calculateRecipeConditions_original = (oils) => {
...
const calc_result_original = () => {
    ...
    sessionStorage.setItem("sapRatio", sap_ratio_global_original.toString());
    sessionStorage.setItem("alkaliRatio", alkali_ratio_global_original.toString());
    sessionStorage.setItem("alcoholRatio", alcohol_ratio_global_original.toString());
...
const display_options_original = () => {
    let features = [];
    for (let i = 0; i < 5; i++) {
        const select = document.getElementById(`select${i + 1}`);
        if (select.value !== "特長を選択") {
...
const initPage = () => {
    for (let i = 1; i <= 5; i++) {
        ...
        if (select) {
            const saved = sessionStorage.getItem(`original_select${i}`);
            if (saved) {
                select.value = saved;
            }
            ...
            select.addEventListener("change", () => {
                sessionStorage.setItem(`original_select${i}`, select.value);
                handleEvent(i);
            });
        }
    }
