...
let cover;
let sap_ratio_global_recommend = 0;
let alkali_ratio_global_recommend = 0;
let alcohol_ratio_global_recommend = 0;
...
const calc_alkali_recommend = (recipe, sap_values, amounts) => {
    const discount = Number(recipe.saponification_rate);
    const alkali_rate = Number(recipe.alkali_purity);

    // 固形せっけん固定
    const result = calc_soda_recommend(sap_values, amounts, discount, alkali_rate);
    sap_ratio_global_recommend = discount;
    alkali_ratio_global_recommend = alkali_rate;
    return result;
};

const calc_water_recommend = (recipe, total, alkali) => {
    const water = Math.round(total * Number(recipe.water_ratio) * 10) / 10;
    return water;
};

const get_oil_names_recommend = (recipe, amounts, total) => {
    let oil_names = [];
...
const get_features_recommend = (recipe) => {
    let skin = 0;
    let clean = 0;
    let foam = 0;
    let hard = 0;
...
const get_option_adjustments_recommend = (recipe) => {
...
const get_final_features_recommend = (recipe) => {
    let features = get_features_recommend(recipe); // オイルのみの計算結果
    let option_adjustments = get_option_adjustments_recommend(recipe); // オプションの影響値
...
const calculateRecipeConditions_recommend = (selectedOils) => {
...
// 要らないキーのみ削除
function clear_preserveSession_recommend() {
    const keysToRemove = [
        "scene",
        "prev_page",
        "id",
...
const calc_result_recommend = (recipe, total) => {
    let sap_values = [];
    let amounts = [];
    for (let i = 0; i < recipe.oils.length; i++) {
        const oil_name = recipe.oils[i].name;
    ...
    clear_preserveSession_recommend();
    ...
    sessionStorage.setItem("sapRatio", sap_ratio_global_recommend.toString());
    sessionStorage.setItem("alkaliRatio", alkali_ratio_global_recommend.toString());
    sessionStorage.setItem("alcoholRatio", alcohol_ratio_global_recommend.toString());
...
const display_recipes_recommend = () => {
    ...
    Object.values(recommend_recipes).forEach((recipe) => {
        ...
        button.addEventListener("click", () => {
            ...
            input_form_button.addEventListener("click", () => {
                ...
                calc_result_recommend(recipe, total);
            });
        });
